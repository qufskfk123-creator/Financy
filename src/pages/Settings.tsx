...
import { Sun, Moon, Trash2, Info, User, Palette } from 'lucide-react'
...
export default function Settings({ theme, onTheme }: Props) {
  ...
  return (
    <div className="p-4 md:p-8 space-y-5 max-w-xl">
      {/* Header */}
      <div>
      ...
      <Section title="데이터 관리">
        <Row label="거래 내역 초기화" sub="포트폴리오 자산은 유지됩니다">
          <button
            ...
            className="flex items-center gap-1.5 px-3.5 py-1.5 rounded-xl bg-amber-500/10 border border-amber-500/30 text-amber-400 hover:bg-amber-500/20 text-xs font-semibold transition-all"
          >
        ...
        <Row label="전체 데이터 삭제" sub="포트폴리오 + 거래 내역 모두 삭제">
          <button
            ...
            className="flex items-center gap-1.5 px-3.5 py-1.5 rounded-xl bg-rose-500/10 border border-rose-500/30 text-rose-400 hover:bg-rose-500/20 text-xs font-semibold transition-all"
          >
            <Trash2 className="w-3 h-3" />
            전체 삭제
          </button>
      ...
      <Section title="앱 정보">
        ...
        <div className="text-[11px] text-gray-700 leading-relaxed">
          모든 데이터는 브라우저 localStorage에 저장됩니다.<br />
