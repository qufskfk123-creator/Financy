...
- **TodayEconAlert**: `ev.event.replace(/^\d{4}-\d{2}-\d{2}\s+/, '')` — Finnhub 이벤트명 날짜 접두사 제거 후 `translateEconEvent()` 적용
  - 이벤트 행 레이아웃: 상단=`[국기][불꽃][지표명 전체폭][배지]`, 하단=`[시간 ET][이전][예상][실제]`
  - 시간은 `utcToET(dateStr)` 로 ET 변환 후 하단 행에 표시 — 시간 없으면 하단 행에도 미표시
  - `utcToET()`: `Date.UTC() + toLocaleTimeString('America/New_York')` — DST(EDT/EST) 자동 처리
- **경제지표 한국어 번역** (`translateEconEvent`):
  - `ECON_NAME_MAP`: 62개 지표 영→한 매핑 (length 내림차순 정렬, 긴 구문 우선 매칭)
  - `SUFFIX_MAP`: YoY→전년 대비, MoM→전월 대비, QoQ→전분기 대비, Weekly→주간, Flash→속보 등 14개
  - 미매핑 지표는 영어 유지, 24자 초과 시 `…` 생략 처리
...
19. **Finnhub 경제캘린더 시간 파싱**: `e.time`이 전체 datetime 형식일 수 있음 → `String(e.time).match(/\d{2}:\d{2}(?::\d{2})?/)` 로 HH:MM:SS만 추출 (api/economic-calendar.ts). Dashboard `utcToET()`로 ET 변환 후 하단 행 표시
20. **경제캘린더 이벤트명**: Finnhub `event` 필드에 날짜 접두사 포함될 수 있음 → 날짜 제거 후 `translateEconEvent()` 로 한국어 변환. 미매핑 시 영어 유지 + 24자 초과 생략
21. **경제캘린더 시간대**: Finnhub 시간은 UTC → `utcToET(dateStr)` 로 미국 동부시간(ET) 변환 표시. `America/New_York` 사용으로 EDT/EST DST 자동 처리
